...
*/
[[gnu::aligned(DEFAULT_ALIGNMENT)]]
FORCE_INLINE const byte_entries_t* get_entry_t_by_bytes(_Atomic(entry_table_t)* atomic_table, const size_t idx, const size_t bytes, const unsigned char inuse) {
 ...
 * @param inuse: The desired flag which can be 0x0 or 0x01 
*/
FORCE_INLINE const offset_entries_t* get_entry_t_by_offset(_Atomic(entry_table_t)* atomic_table, const size_t idx, const size_t offset, const unsigned char inuse) {
    const entry_table_t table = atomic_load_explicit(atomic_table, memory_order_relaxed);
    ...
    if (table->byte_entries && table->offset_entries) {
        for (size_t i = 0; i < table->bucket_count; i++) {
            ...
            if (arr) {
                ...
                if (table->byte_entries[i]) munmap_address(table->byte_entries[i], byte_row_size, __FILE__,  __LINE__);
...
FORCE_INLINE unsigned char is_mergeable(_Atomic(entry_table_t)* atomic_table, const size_t idx, const size_t bytes) {
    ...
    const byte_entries_t** arr = (const byte_entries_t**)table.byte_entries[idx];
    if (!arr) return 0x0;

    for (size_t slot = 0; slot < table.inner_count[idx]; slot++) {
        const byte_entries_t* seed = arr[slot];
            ...
            if (seed->inuse == 0x0 && seed->offset) {
                ...
                if (seed->offset->offset) {
                    ...
                    while (accumulated < bytes) {
                        const offset_entries_t* nxt = get_entry_t_by_offset(atomic_table, idx, probe_offset, 0x0);
                        if (!nxt) break;
...
[[gnu::flatten]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]]
GCC_OPTIMIZE_O0 void merge(blocks_t* blocks, const size_t idx, const size_t bytes) {
    ...
    for (size_t slot = 0; slot < table.inner_count[idx] && !found; slot++) {
        ...
        while (__builtin_expect(seed != NULL, 1)) {
            ...
            if (seed->inuse == 0x0 && seed->offset) {
                ...
                size_t probe_offset = seed->offset->offset + seed->bytes;
                while (accumulated < bytes) {
                    const offset_entries_t* nxt = get_entry_t_by_offset(&blocks->table, idx, probe_offset, 0x0);
                    if (!nxt) break;
                    accumulated += nxt->bytes->bytes;
                    probe_offset += nxt->bytes->bytes;
                }
    ...
    while (total < bytes) {
        const offset_entries_t* oe = get_entry_t_by_offset(&blocks->table, idx, cur_offset, 0x0);
        if (!oe) break;
...
[[gnu::aligned(DEFAULT_ALIGNMENT)]]
FORCE_INLINE const blocks_t* get_block_t_by_offset(blocks_t* blocks, const size_t idx, const size_t offset, const unsigned char inuse) {
...
GCC_OPTIMIZE_O0 void* coalescing(size_t bytes); /* Defined in Allocator Section */
...
FORCE_INLINE bucket_t* find_slot(void* ptr) {
    ...
    for (size_t i = 0; i < BUCKET_SMALL_CAP; i++) {
        ...
        if (b->arena && p >= (uintptr_t)b->arena->chunk && p < (uintptr_t)b->arena->chunk + ARENA_SIZE) {
            ...
            const blocks_t* blocks = get_block_t_by_offset(&b->blocks, i, offset, 0x01);
            if (blocks) return b;
        }
    }
...
[[gnu::pure]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]]
FORCE_INLINE huge_slot_t* find_huge_slot(const void* ptr, const char* chunk, huge_slot_t* slots) {
    const size_t offset = (uintptr_t)ptr - (uintptr_t)chunk;
    huge_slot_t* slot = &slots[0];
    for (size_t i = 1; __builtin_expect(slot != NULL, 1); i++) {
        __builtin_prefetch(&slots[i], 0, 1);
        slot = &slots[i - 1];
        const offset_entries_t* onode = get_entry_t_by_offset(&slot->blocks.table, i, offset, 0x01);
        if (onode) return slot;
    }
    return NULL;
}
...
FORCE_INLINE void push_to_bucket(bucket_t* slot, size_t offset) {
    ...
    const offset_entries_t* onode = get_entry_t_by_offset(&slot->blocks.table, idx, offset, 0x01);
    if (!onode) return;
    __builtin_prefetch(onode->bytes, 0, 1);
...
FORCE_INLINE void* pop_from_bucket(bucket_t* slot, const size_t bytes) {
    ...
    const byte_entries_t* bnode = get_entry_t_by_bytes(&slot->blocks.table, idx,  bytes, 0x0);
...
GCC_OPTIMIZE_O0 void init_huge_allocator(void);
...
GCC_OPTIMIZE_O0 size_t allocator_huge_bitmap_ranges(const size_t start, size_t end);
...
[[gnu::cold]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]]
GCC_OPTIMIZE_O0 void allocator_init_arena_t(void) {
...
GCC_OPTIMIZE_O0 void allocator_init_buckets_t(void) {
    int res = 0;
...
[[gnu::cold]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]]
GCC_OPTIMIZE_O0 void allocator_init_threads_t(void) {
    int res = 0;
    if (!allocator.pool) {
        ...
        static const unsigned char mode = 0x0, attr = 0x01, locked = 0x0, cond = 0x0, stack = 0x0;
        static const size_t start = 1, end = ALLOC_THREAD_POOL_SIZE;
        allocator.pool[0] = init_threads_t(mode, attr, locked, cond, stack);
        routine_metadata(&allocator.pool[0], 7, &mode, &attr, &locked, &cond, &stack, &start, &end, &allocator.huge->done);
...
[[gnu::pure]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]]
GCC_OPTIMIZE_O0 void* coalescing(size_t bytes) {
...
[[gnu::cold]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]]
GCC_OPTIMIZE_O0 void debug_allocator(const size_t bytes) {
...
GCC_OPTIMIZE_O0 void deallocate(void* ptr) {
    ...
    if (!slot) {
        ...
        huge_slot_t* huge = find_huge_slot(ptr, allocator.huge->region, allocator.huge->slots);
        if (!huge) return;
        
        size_t idx = huge - allocator.huge->slots;
        size_t offset = (uintptr_t)ptr - (uintptr_t)allocator.huge->region;
        atomic_size_t ast_end;
        
        threads_t* t1 = find_thread_t(allocator.pool, ALLOC_THREAD_POOL_SIZE);
        if (!t1) atomic_store_explicit(&ast_end, allocator_huge_bitmap_ranges(huge->capacity, allocator.huge->slots[allocator.huge->slot_cap - 1].capacity), memory_order_relaxed);
        else {
            // TODO: we are going to need to add the threading here, but we need to fix the creating/join and use the sleep/awake method
        }
        const offset_entries_t* onode = get_entry_t_by_offset(&allocator.huge->slots[idx].blocks.table, idx, offset, 0x01);

        if (!onode) return;
        const size_t subtract = (onode->offset + onode->bytes->bytes);
        allocator.huge->slots[idx].space -= allocator.huge->slots[idx].space - subtract != SIZE_MAX ? subtract : allocator.huge->slots[idx].space;
        entry_table_t table = atomic_load_explicit(&allocator.huge->slots[idx].blocks.table, memory_order_relaxed);
        const size_t end = atomic_load_explicit(&ast_end, memory_order_relaxed);
        if (allocator.huge->slots[idx].space == 0) {
            __builtin_prefetch(onode->bytes, 0, 1);
            allocator.huge->bitmap = allocator.bitmap.bitmap_clear(allocator.huge->bitmap, idx, huge->capacity, end);
            destroy(&allocator.huge->slots[idx].blocks.table, idx, onode->offset, onode->bytes->bytes);
            table_entry_free_pages(&table);
            blocks_t_free_pages(&allocator.huge->slots[idx].blocks);
        } else {
            __builtin_prefetch(onode->bytes, 0, 1);
            allocator.huge->bitmap = allocator.bitmap.bitmap_clear(allocator.huge->bitmap, idx, huge->capacity, end);
            update(&allocator.huge->slots[idx].blocks.table, idx, onode->offset, onode->bytes->bytes, 0x0);
        }
        if (((allocator.huge->region + 0) != ptr) && ((allocator.huge->region + (allocator.huge->allocator_cap + 1)) != ptr)) memset(ptr, 0xFF, onode->bytes->bytes);
        return;
    }
    ...
    const offset_entries_t* offset_entry = get_entry_t_by_offset(&slot->blocks.table, atomic_load_explicit(&index, memory_order_relaxed), atomic_load_explicit(&offset, memory_order_relaxed), 0x01);
...
[[gnu::flatten]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]] /* Align it to a byte-boundry, so it can fit into the cache without an issue */
GCC_OPTIMIZE_O0 void init_allocator_t() {
    #if BENCHMARK_ENV == 1
...
[[gnu::aligned(DEFAULT_ALIGNMENT)]] /* Align it to a byte-boundry, so it can fit into the cache without an issue */
/**
 * @description: Free function similar to that of 'arena' 'push', it will push the bytes to the stack, but if full, 
 * it will recrusive call `allocate` afer marking the slot as occupied
...
// end should be allocator.huge.slots[allocator.huge.slot_cap - 1].capacity
// while start is the `capacity` found from find_huge_slot
[[gnu::hot]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]] /* Align it to a byte-boundry, so it can fit into the cache without an issue */
GCC_OPTIMIZE_O0 size_t allocator_huge_bitmap_ranges(const size_t start, size_t end) {
    const huge_slot_t* slot = &allocator.huge->slots[end];
    if (end < start || end == SIZE_MAX) return SIZE_MAX;
    else if (!slot) return SIZE_MAX;
    else if (allocator.huge->slots[end].capacity != start) return allocator_huge_bitmap_ranges(start, end / 2);
    return end; 
}
...
[[gnu::destructor]]
[[gnu::aligned(DEFAULT_ALIGNMENT)]] /* Align it to a byte-boundry, so it can fit into the cache without an issue */
GCC_OPTIMIZE_O0 void allocator_dctor() {
    while(!atomic_load_explicit(&allocator.huge->done, memory_order_acquire)){}
    for (size_t i = 0;  i < ALLOC_THREAD_POOL_SIZE; i++) clean_threads(&allocator.pool[i]);
    if (allocator.pool) munmap_address(allocator.pool, ALLOC_THREAD_POOL_SIZE * sizeof(threads_t), __FILE__,  __LINE__);
    allocator.pool = NULL;
    bucket_t_dctor();
    if (allocator.arena) {
